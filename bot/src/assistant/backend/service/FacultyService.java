...
public final class FacultyService {
 ...
 private List<FacultyDTO> faculty(int page,int size,String search){String p="/api/v1/inelicom/faculties?page_index="+page+"&limit="+size; return BackendClient.getData(p).map(n->n.path("items")).stream().flatMap(n->StreamSupport.stream(n.spliterator(),false)).map(this::map).toList();}
