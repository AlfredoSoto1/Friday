...
public final class FacultyService {
  public long getRecordCount(String department) {
    return faculty(0, 100, department).size();
  }
  ...
  public List<FacultyDTO> getFaculty(int page, int size, String department) {
    return faculty(page, size, department);
  }
  ...
  public int addProfessor(FacultyDTO p) {
    return 0;
  }

  private List<FacultyDTO> faculty(int page, int size, String search) {
    String p = "/api/v1/inelicom/faculties?page_index=" + page + "&limit=" + size;
  ...
  private FacultyDTO map(JsonNode n) {
    FacultyDTO d = new FacultyDTO();
    d.setId(n.path("faculty_id").asInt());
    d.setName(n.path("name").asText());
    d.setDescription("");
    d.setJobentitlement("Faculty");
    d.setOffice("");
