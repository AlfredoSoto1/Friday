...
public final class OrganizationsService {
  ...
  public Optional<OrganizationDTO> getOrganization(String name) {
    return organizations(0, 25, name).stream()
  ...
  private List<OrganizationDTO> organizations(int page, int size, String search) {
    ...
    return BackendClient.getData(path).map(d -> d).stream()
        .flatMap(n -> StreamSupport.stream(n.spliterator(), false))
        .map(this::map)
        .toList();
  }

  private OrganizationDTO map(JsonNode n) {
    OrganizationDTO d = new OrganizationDTO();
    d.setId(n.path("organization_id").asInt());
    d.setName(n.path("name").asText());
    d.setDescription(n.path("description").asText());
    return d;
  }
}
