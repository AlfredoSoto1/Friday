...
public class RulesCmd extends InteractionModel implements CommandI {
  ...
  private void fromServer(SlashCommandInteractionEvent event) {
    ...
    Optional<Role> conRole =
        super.getEffectiveRole(MemberPosition.CONSEJERO_PROFESIONAL, event.getGuild());
    ...
    String admRole =
        event.getGuild().getRolesByName("Administrator", true).stream()
            .findFirst()
    ...
    event
        .replyEmbeds(
            ...
            embed.buildServerUsageRules(
                color,
                roleMention(bdeRole, "Bot Developer"),
                roleMention(modRole, "Moderator"),
                roleMention(staRole, "Staff")),
            embed.buildBotUsageRules(
                color,
                roleMention(bdeRole, "Bot Developer"),
                roleMention(esoRole, "Estudiante Orientador")))
        .queue();
  ...
  private void fromDM(SlashCommandInteractionEvent event) {
    event
        .replyEmbeds(
            ...
            embed.buildServerUsageRules(0x808080, "BotDeveloper", "Moderador", "Staff"),
            embed.buildBotUsageRules(0x808080, "BotDeveloper", "Estudiante Orientador"))
        .queue();
  }
}
