...
public class ProjectsCmd extends InteractionModel implements CommandI {
  ...
  public List<OptionData> getOptions(Guild server) {
    OptionData projects =
        new OptionData(OptionType.STRING, COMMAND_LABEL, "Selecciona el proyecto", true).setAutoComplete(true);
  ...
  public List<Command.Choice> getAutoCompleteChoices(String optionName, String input) {
    ...
    return service.getProjects(input).stream()
  ...
  public void fromServer(SlashCommandInteractionEvent event, String selectedProject) {
    ...
    if (project.isPresent()) {
      event
          .replyEmbeds(embed.buildProject(color, project.get()))
