...
public class GamesCmd extends InteractionModel implements CommandI, MessengerI {
  ...
  public List<OptionData> getOptions(Guild server) {
    return List.of(
        new OptionData(
                OptionType.STRING,
                "disable-smart-response",
                "disables/enables smart response",
                false)
  ...
  public void execute(SlashCommandInteractionEvent event) {
    ...
    service.updateCommandUserCount(
  ...
  private void handlePredefinedMessages(String userMessage, TextChannel channel, Message message) {
    ...
    if (!smartResponseEnabled) return;
  ...
  private void processUserXP(Message message, TextChannel channel, Guild server) {
    ...
    Optional<UserRankDTO> userRank =
        service.giveXP(message.getAuthor().getName(), 1, server.getIdLong());

    userRank.ifPresent(
        rank -> {
          if (!rank.isHasLevelup()) return;
