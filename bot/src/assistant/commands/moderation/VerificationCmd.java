...
public class VerificationCmd extends InteractionModel implements CommandI {
	...
	public void execute(SlashCommandInteractionEvent event) {
		...
		if (textChannel.isPresent()) {
			...
			textChannel.get()
					.sendMessageEmbeds(verificationEmbed.buildVerificationPrompt(modRole.get(), bdeRole.get(), color))
	...
	private void verifyUser(ModalInteractionEvent event) {
		...
		if (memberDTO.get().isVerified()) {
			...
			event.getMember().getUser()
					.openPrivateChannel()
					.queue(privateMessage -> showWelcomeMessage(privateMessage, memberDTO.get(), event.getGuild()));
		}
	...
	private void applyRoles(InteractionHook hook, Guild server, Member member, String email) {
		...
		for (DiscordRoleDTO roleDTO : classificationRoles) {
			...
			if (role == null) {
				Logger.instance().logFile(LogFeedback.WARNING, "Configured Discord role no longer exists: %s",
						roleDTO.getRoleid());
				continue;
			}
			...
			try {
				server.addRoleToMember(member, role).queue(
						success -> Logger.instance().logFile(LogFeedback.SUCCESS, "Given Role [%s] to [%s]", role.getName(),
								member.getEffectiveName()),
						error -> Logger.instance().logFile(LogFeedback.ERROR, "Failed giving Role [%s] to [%s]", role.getName(),
								member.getEffectiveName()));

				delayInteractions(2000); // two seconds
			} catch (HierarchyException he) {
				hook.sendMessage("No se pudo otorgar los roles, por favor notifique a un Bot developer").setEphemeral(true)
						.queue();
				Logger.instance().logFile(LogFeedback.WARNING, "Failed to add role: %s to member %s", he.getMessage(),
						member.getEffectiveName());
			}
		}
	}
	...
	private void applyTeam(InteractionHook hook, Guild server, Member member, String email) {
		...
		if (team.isEmpty()) {
			hook.sendMessage("No se pudo otorgar el equipo, por favor notifique a un Bot developer").setEphemeral(true)
					.queue();
	...
	private void applyNickname(InteractionHook hook, Guild server, Member member, MemberDTO memberDTO) {
		...
		try {
			server.modifyNickname(member, nickname).queue(
					success -> Logger.instance().logFile(LogFeedback.SUCCESS, "Successfully changed nickname from [%s] to [%s]",
							member.getEffectiveName(), nickname),
					error -> Logger.instance().logFile(LogFeedback.ERROR, "Failed changing nickname from [%s] to [%s]",
							member.getEffectiveName(), nickname));

			delayInteractions(2000); // two seconds
		} catch (HierarchyException he) {
			hook.sendMessage("No se pudo otorgar el nickname, por favor notifique a un Bot developer").setEphemeral(true)
					.queue();
