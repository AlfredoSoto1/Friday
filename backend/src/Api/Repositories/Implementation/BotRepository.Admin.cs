...
public sealed partial class BotRepository
{
  ...
  public async Task<Result<BotChannel, AppError>> CreateGuildChannel(IDbConnection connection, IDbTransaction transaction, long guildId, BotChannelRequest request)
  {
    try
    {
      const string sql = @"
        ...
        INSERT INTO discord.channels (server_id, discord_channel_id, parent_channel_id, name, type, position, topic, nsfw)
        SELECT server_id, @DiscordChannelId, @ParentChannelId, @Name, @Type, @Position, @Topic, @Nsfw
          FROM target_server
        RETURNING channel_id, (SELECT guild_id FROM target_server) AS guild_id, discord_channel_id, parent_channel_id,
                  name, type, position, topic, nsfw, created_at, updated_at;
      ";
      ...
      return record is null
        ? Result<BotChannel, AppError>.Fail(AppError.NotFound($"Guild with ID {guildId} not found."))
        : Result<BotChannel, AppError>.Ok(MapToBotChannel(record));
    }
    catch (Exception ex)
    {
      return Result<BotChannel, AppError>.Fail(AppError.BadRequest(ex.Message));
    }
  }

  public async Task<Result<BotChannel, AppError>> UpdateGuildChannel(IDbConnection connection, IDbTransaction transaction, long guildId, int channelId, BotChannelRequest request)
  {
    try
    {
      const string sql = @"
        UPDATE discord.channels
           SET discord_channel_id = @DiscordChannelId,
               ...
               updated_at = CURRENT_TIMESTAMP
          FROM discord.servers
        WHERE discord.channels.server_id = discord.servers.server_id
          AND discord.servers.guild_id = @GuildId
          AND discord.channels.channel_id = @ChannelId
        RETURNING discord.channels.channel_id, discord.servers.guild_id, discord.channels.discord_channel_id,
                  discord.channels.parent_channel_id, discord.channels.name, discord.channels.type,
                  discord.channels.position, discord.channels.topic, discord.channels.nsfw,
                  discord.channels.created_at, discord.channels.updated_at;
      ";
      ...
      return record is null
        ? Result<BotChannel, AppError>.Fail(AppError.NotFound("Channel not found."))
        : Result<BotChannel, AppError>.Ok(MapToBotChannel(record));
    }
    catch (Exception ex)
    {
  ...
  public async Task<Result<bool, AppError>> DeleteGuildChannel(IDbConnection connection, IDbTransaction transaction, long guildId, int channelId)
  {
    try
    {
      const string sql = @"
        DELETE FROM discord.channels
          USING discord.servers
        WHERE channels.server_id = servers.server_id
          AND servers.guild_id = @GuildId
          AND channels.channel_id = @ChannelId;
      ";

      var affected = await connection.ExecuteAsync(sql, new { GuildId = guildId.ToString(), ChannelId = channelId }, transaction);
      return affected == 0
        ? Result<bool, AppError>.Fail(AppError.NotFound("Channel not found."))
        : Result<bool, AppError>.Ok(true);
    }
    catch (Exception ex)
    {
